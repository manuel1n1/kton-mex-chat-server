...
import com.manuel1n1.models.response.UserData
    ...
    route("/auth") {
        post("/login") {
            try {
                ...
                val userExist = userDao.findByEmail(user.userName)
                if(userExist == null)
                    ...
                    call.respond(UserLogin(UserData(userExist.id, userExist.email), token, token))
                }
            } catch (ex: Exception) {
